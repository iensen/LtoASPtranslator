...
can also be written as $A, B$. Parentheses can be omitted in which case $and$ has higheer precedence than $or$. A sentence is called a \textbf{\textit{predicate sentence}}, if all the atoms occurring in the sentence are predicate atoms.

\subsection{Maybe Statements}

A \textbf{\textit{maybe statement}} is of the form $$\mbox{\texttt{maybe}}~p(t_1,\ldots t_n)$$ where $p(t_1,\ldots,t_n)$ is a basic predicate atom.
...
We will also refer to $head$ and $sentence$ and the \textit{\textbf{head}} and the \textit{\textbf{body}} of the corresponding rule respectively.
...
\subsection{Programs  Variables and Ground Programs}\label{grp}
...
\noindent
Let $S$ be a sentence, and 
\noindent\medskip
Let
\begin{itemize}
\item $\{U_1,\ldots U_n\}$ be the set of all universally quantified terms  of $S$ whose  types are $Tu_1,\ldots, Tu_n$ respectively;
\item $\{E_1, \ldots E_m\}$ be the set of all universally quantified terms  of $S$ whose  types are $Te_1,\ldots, Te_m$ respectively;
\end{itemize}

\noindent\medskip
For a set $\{T_1,\ldots,T_k\}$ of quantified terms of $S$  $S|_{\{T_k = t_1, \ldots, T_k = t_k\}}$ we denote the sentence obtained from $S$ by 
\begin{enumerate}
\item for quantified term $T_i \in \{T_1,\ldots, T_k\}$ replacing every occurrence of a quantified term $T_i$  with $t_i$;
\end{enumerate}
...
\begin{enumerate}
\item replacing all maybe statements of the form $\texttt{maybe}~l$ with $l~or~(\texttt{not}~l)$.
