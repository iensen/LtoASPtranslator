...
A shorthand $\{l..r\}$, where each of $l$ and $r$ is either a constant name, integer numeral or an arithmetic term may be used to represent the  set of all integers in the range $n..m$.
...
A \textit{\textbf{sentence}} is either an atom or an expression of the form $(not~A)$, $(A~or~B)$, $(A~and~B)$, where $A$ and $B$ are atoms. Parentheses can be omitted in which case $not$ has highest precedence and $or$ has the lowest precedence. 
...
$arithmetic\_term$ ::= $-(T0) | (T0~infix_1~T1)|(T1~infix_2~T2)| T0~infix_1~T1|T1~infix_2~T2$\\
$infix_1$ ::= $ \texttt{+} | \texttt{-} $\\
$infix_2$ ::= $ \texttt{*} | / | \% $\\
...
$const\_decl$ ::= $identifier \texttt{=} arithmetic\_term.~|~identifier \texttt{=} identifier.~$\\
~~~~~~~~~~$~~~~~~~~~~~~~~~~~~~~|~identifier \texttt{=} numeric\_constant.$
\subsubsection{Type Declarations}

$type\_decl$ ::= $identifier~ \texttt{=} ~set\_expr.$\\
$limit$ ::= $identifier~|~numeric\_constant~|~arithmetic\_term$\\
$set$ ::= $\{ [basic\_terms] \}$\\
$range$ ::= $\{limit..limit\}$\\
$set\_expr$ ::= $ST0 $ \\  
...
$ST1$ ::= $ST2~|~ST1~*~ST2~|~ST1~\backslash~ST2$ \\
$ST2$ ::= $(ST0)~|~set~|~range~|~set\_constr~|~identifier$  \\

\subsubsection{Atoms}
$atom$::= $identifier [( terms )] | built\_in $\\
