...
%DIF DEL Lspec_old.tex   Mon Feb 22 21:23:17 2016
%DIF ADD Lspec.tex       Fri Mar 11 23:40:01 2016
...
\item $(t \diamond u)$
\end{itemize} 
where each of $t$ and $u$ is   either an integer numeral, a variable  \DIFdelbegin \DIFdel{, a numeric constant  }\DIFdelend or an  arithmetic \DIFdelbegin \DIFdel{terms}\DIFdelend \DIFaddbegin \DIFadd{term}\DIFaddend ; and $\diamond$ is a special symbol in the set \texttt{\{'+', '-', '*', '/', '\%'\}}; (\texttt{'\%'} stands for modulo operation). Parentheses can optionally be omitted in which case standard operator precedences apply. 
...
\DIFaddbegin \DIFadd{We say that $t^\prime$ is a }\textit{\DIFadd{proper subterm}} \DIFadd{of $t$ if $t^\prime$ is a subterm of $t$ and $t \not= t^\prime$. 
}

\DIFaddend \noindent
A term $t$ is called \textbf{\textit{ground}} iff \DIFaddbegin \DIFadd{one of the following holds:
...
\begin{itemize}
\item \DIFadd{$a$
}\item \DIFadd{$\texttt{not}~a$
}\end{itemize}
\DIFadd{where $a$ is an atom.
}
...
can also be written as $A, B$. Parentheses can be omitted in which case \DIFdelbegin \DIFdel{$not$ has highest precedence and }\DIFdelend \DIFaddbegin \DIFadd{$and$ has higheer precedence than }\DIFaddend $or$\DIFdelbegin \DIFdel{has the lowest precedence}\DIFdelend . A sentence is called a \textbf{\textit{predicate sentence}}, if all the atoms occurring in the sentence are predicate atoms.

\subsection{Maybe \DIFdelbegin \DIFdel{Literals}\DIFdelend \DIFaddbegin \DIFadd{Statements}\DIFaddend }

A \textbf{\DIFdelbegin \textit{\DIFdel{maybe literal}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \textit{\DIFadd{maybe statement}}\DIFaddend } is of the form $$\mbox{\texttt{maybe}}~p(t_1,\ldots t_n)\DIFdelbegin \DIFdel{.}\DIFdelend $$ where $p(t_1,\ldots,t_n)$ is a basic predicate atom.
...
\item  a maybe \DIFdelbegin \DIFdel{literal}\DIFdelend \DIFaddbegin \DIFadd{statement}\DIFaddend ;
\item  a cardinality constraint.
\end{enumerate}

\DIFaddbegin \DIFadd{and $sentence$ is a sentence defined in section (\ref{sentsec}).
We will also refer to $head$ and $sentence$ and the }\textit{\textbf{\DIFadd{head}}} \DIFadd{and the }\textit{\textbf{\DIFadd{body}}} \DIFadd{of the corresponding rule respectively.
 }
...
\item The leftmost occurrence of any variable  $V$ in a rule \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{must be in the head of the rule and must be }\DIFaddend preceeded by a type name (also referred to as \textit{the type} of the variable \DIFdelbegin \DIFdel{), which can also be preceed by a quantifier}\DIFdelend \DIFaddbegin \DIFadd{in this rule)}\DIFaddend .
...
\DIFadd{$literal$ }\DIFaddend ::= \DIFdelbegin \DIFdel{$s3$ }\DIFdelend \DIFaddbegin \DIFadd{$atom~|~\texttt{not}~atom$}\DIFaddend \\
\DIFdelbegin \DIFdel{$s3$ }\DIFdelend \DIFaddbegin \DIFadd{$predicate\_literal$ }\DIFaddend ::= \DIFdelbegin \DIFdel{$s2~|~s3~\texttt{or}~s2$}\DIFdelend \DIFaddbegin \DIFadd{$atom~|~\texttt{not}~predicate\_atom$
}
...
\DIFdel{with }\begin{displaymath}\DIFdel{ quantifier~p~X}\end{displaymath}
 %DIFAUXCMD
\DIFdel{where $X$ is a variable not occurring in $S$ and 
unique for each quantified term in $S$.
}%DIFDELCMD < 
...
}\DIFaddend \item \DIFdelbegin \DIFdel{$Y_1, \ldots Y_m$ be the variables occurring in }\DIFdelend \DIFaddbegin \DIFadd{$\{E_1, \ldots E_m\}$ be the set of all }\DIFaddend universally quantified terms  of \DIFdelbegin \DIFdel{$S^\prime$ of types $Ty_1, \ldots, Ty_m$ correspondingly
}\DIFdelend \DIFaddbegin \DIFadd{$S$ whose  types are $Te_1,\ldots, Te_m$ respectively;
}\DIFaddend \end{itemize}

\noindent\medskip
For a \DIFdelbegin \DIFdel{sentence }\DIFdelend \DIFaddbegin \DIFadd{set $\{T_1,\ldots,T_k\}$ of quantified terms of }\DIFaddend $S$  \DIFdelbegin \DIFdel{by $S|_{\{Z_1 = z_1, \ldots, Z_n = z_n\}}$ }\DIFdelend \DIFaddbegin \DIFadd{$S|_{\{T_k = t_1, \ldots, T_k = t_k\}}$ }\DIFaddend we denote the sentence obtained from $S$ by 
\begin{enumerate}
\item for \DIFdelbegin \DIFdel{each variable $Z_i \in \{Z_1,\ldots, Z_n\}$ }\DIFdelend \DIFaddbegin \DIFadd{quantified term $T_i \in \{T_1,\ldots, T_k\}$ }\DIFaddend replacing every occurrence of a quantified term \DIFdelbegin \begin{displaymath}\DIFdel{ quantifier~p~Z_i}\end{displaymath} %DIFAUXCMD
\DIFdel{with $z_i$;
}%DIFDELCMD < \item %%%
\DIFdel{replacing all other occurrences of each $Z_i \in  \{Z_1,\ldots, Z_n\}$ with $z_i$.
}\DIFdelend \DIFaddbegin \DIFadd{$T_i$  with $t_i$;
...
\begin{enumerate}
\item \DIFadd{replacing all maybe statements of the form $\texttt{maybe}~l$ with $l~or~(\texttt{not}~l)$.
